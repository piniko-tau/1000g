...
#
    if args.mind_update_table_allel2peptide:
        ...
        for hg2 in query2list():
            ...
            print(cur.mogrify("update %s ,t1||t2||t3 as t12 from (select szptcr0001, case when substr(szptcr0001,1,1) = allele1 then '+'||peptide1 when substr(szptcr0001,1,1) = allele2 then '+'||peptide2 when substr(szptcr0001,1,1) = allele3 then '+'||peptide3 when substr(szptcr0001,1,1) = opallele1 then '-'||peptide1 when substr(szptcr0001,1,1) = opallele2 then '-'||peptide2 when substr(szptcr0001,1,1) = opallele3 then '-'||peptide3 end as t1 , case when substr(szptcr0001,3,1) = allele1 then '+'||peptide1 when substr(szptcr0001,3,1) = allele2 then '+'||peptide2 when substr(szptcr0001,3,1) = allele3 then '+'||peptide3 when substr(szptcr0001,3,1) = opallele1 then '-'||peptide1 when substr(szptcr0001,3,1) = opallele2 then '-'||peptide2 when substr(szptcr0001,3,1) = opallele3 then '-'||peptide3 end as t2 ,case when substr(szptcr0001,5,1) = '' then '' when substr(szptcr0001,5,1) = allele1 then '+'||peptide1 when substr(szptcr0001,5,1) = allele2 then '+'||peptide2 when substr(szptcr0001,5,1) = allele3 then '+'||peptide3 when substr(szptcr0001,5,1) = opallele1 then '-'||peptide1 when substr(szptcr0001,5,1) = opallele2 then '-'||peptide2 when substr(szptcr0001,5,1) = opallele3 then '-'||peptide3 end as t3 from mind_data_1ann) as mytable;
